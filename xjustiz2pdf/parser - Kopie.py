...
class AktenParser:
    ...
    def parse(self, xml_path: str) -> DocNode:
        ...
        container = root_el.find(f".//{self.TAG_SCHRIFTOBJEKTE}") or root_el
    ...
    def _parse_document(self, el) -> List[DocNode]:
        ...
        for datei in el.findall(f".//{self.TAG_DATEI}"):
            ...
            bestandteil = datei.find(f"{self.TAG_BESTANDTEIL}") or datei.find(f".//{self.TAG_BESTANDTEIL}")
            if bestandteil is not None:
                code_el = bestandteil.find("code")
                if code_el is None:
                    code_el = bestandteil.find(f"{self.NS}code") or bestandteil.find(f".//{self.NS}code")

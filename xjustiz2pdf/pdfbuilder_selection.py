...
def build_state_accessor(check_states: dict[str, int], disabled_paths: set[str]) -> Callable[[Optional[str]], int]:
    """
    ...
    """
    def state_for_path(path: Optional[str]) -> int:
        if path and path in disabled_paths:
            debug(f"[StateAccessor] path='{path}' ist disabled → Unchecked")
            return Qt.Unchecked
        state = check_states.get(path or "", Qt.Unchecked)
        debug(f"[StateAccessor] path='{path}' → state={state}, keys_sample={list(check_states.keys())[:5]}")
        return state
    return state_for_path


def get_root_docs_and_state(root: DocNode, state_for_path: Callable[[Optional[str]], int]) -> Tuple[List[DocNode], int]:
    """
...
def collect_docs_flat(root: DocNode, state_for_path: Callable[[Optional[str]], int],
                      filter_terms: List[str], only_originals: bool) -> List[DocNode]:
    """
    ...
    def walk(node: DocNode, path_here: str):
        state = state_for_path(path_here)
        debug(f"[Selection] Walk Folder={node.anzeigename}, Path={path_here}, State={state}")
        if node.is_folder():
            if state == Qt.Checked:
                for c in node.children:
                    if c.is_doc() and not should_filter(c):
